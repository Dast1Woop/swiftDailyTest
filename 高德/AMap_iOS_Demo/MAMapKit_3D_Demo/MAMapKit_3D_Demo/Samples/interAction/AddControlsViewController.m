...
- (void)viewDidLoad
{
    ...
    self.mapView = [[MAMapView alloc] initWithFrame:CGRectMake(0, 90, self.view.bounds.size.width, self.view.bounds.size.height - 90)];
...
- (IBAction)switchShowScale:(id)sender {
    UISwitch *theSwitch = (UISwitch *)sender;
    self.mapView.showsScale = theSwitch.on;
}

//是否显示指南针，此外还可以改变指南针原点位置，指南针的宽高，详见MAMapView头文件
- (IBAction)switchShowCompass:(id)sender {
    UISwitch *theSwitch = (UISwitch *)sender;
    self.mapView.showsCompass = theSwitch.on;
}

//是否显示建筑物，详见MAMapView头文件
- (IBAction)switchShowBuilding:(id)sender {
    UISwitch *theSwitch = (UISwitch *)sender;
    self.mapView.showsBuildings = theSwitch.on;
}

//是否显示文字，详见MAMapView头文件
- (IBAction)switchShowLabels:(id)sender {
    UISwitch *theSwitch = (UISwitch *)sender;
